...
class CommandProcessor:
    ...
    def __init__(self, context: cxt.Context, connection: comms.HttpConnection):
        ...
        self._use_json = context.use_json()
    ...
    def _dump_to_log(self, data: str | dict | list | tuple) -> bool:
        if self._use_json:
            data = data.split('\n') if isinstance(data, str) else data
            data = json.dumps(data, indent=2, separators=(',', ': ')).split('\n')
        else:
    ...
    def _metrics(self) -> bool:
        return self._dump_to_log(self._connection.get('/metrics'))

    def _mprof(self) -> bool:
        return self._dump_to_log(self._connection.get('/mprof'))

    def _modules(self) -> bool:
        result = self._connection.get('/modules')
        if not self._use_json:
            result = tuple(self._connection.get('/modules').keys())
        return self._dump_to_log(result)

    def _instances(self) -> bool:
        if self._use_json:
            return self._dump_to_log(self._instances)
    ...
    def _runtime_meta(self) -> bool:
        result = self._connection.get(self._instance_path('/deployment/runtime-meta'), 'Runtime Not Found')
        return self._dump_to_log(result)

    def _world_meta(self) -> bool:
        return self._dump_to_log(self._connection.get(self._instance_path('/deployment/world-meta')))
    ...
    def _wipe_world(self, argument: str | None) -> bool:
        if not argument:
            return self._dump_to_log(util.get_resource('wipeworldhelp.text').strip())
    ...
    def _print(self, argument: str) -> bool:
        return self._dump_to_log(argument if argument else '')
    ...
    def _showtoken(self) -> bool:
        url, token = self._url.replace('localhost', util.get_local_ip4()), self._token
        if self._use_json:
            return self._dump_to_log(dict(url=url, token=token))
        logging.info(util.OUT + 'URL: ' + url)
        logging.info(util.OUT + 'Token: ' + token)
        return True
    ...
    def _server(self, argument: str | None) -> bool:
        if argument and argument != 'status':
            ...
            return True
        return self._dump_to_log(self._connection.get(self._instance_path('/server')))
    ...
    def _system(self) -> bool:
        return self._dump_to_log(self._connection.get('/system/info'))

    def _report(self) -> bool:
        identities = self._instances.keys()
        if len(identities) == 0:
            return True
        result, original = {}, self._instance
        for identity in identities:
            self._instance = identity
            result[identity] = self._connection.get(self._instance_path('/server'))
            del result[identity]['instance']
    ...
    def _log_tail_f(self) -> bool:
        self._log_tail('10')
        self._connection.drain(self._connection.post(self._instance_path('/log/subscribe')))
        return True

    def _console_help(self) -> bool:
        return self._dump_to_log(self._connection.get(self._instance_path('/console/help')))
    ...
    def _console_send(self, argument: str) -> bool:
        ...
        return self._dump_to_log(result.strip())
    ...
    def _https(self, argument: str | None) -> bool:
        if not argument:
            return self._dump_to_log(self._connection.get('/ssl'))
    ...
    def _steamcmd_reset(self) -> bool:
        return self._dump_to_log(self._connection.post('/system/steamcmd/reset'))
