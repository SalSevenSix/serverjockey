...
if [ -z $SOURCE_ZIP ]; then
  ...
  echo "Unpacking zip"
  cd $DIST_DIR || exit 1
  unzip "$SOURCE_ZIP" > /dev/null || exit 1
  mv $(ls -d serverjockey-*/ | cut -f1 -d'/') "$SERVERJOCKEY_DIR" || exit 1
...
$SERVERJOCKEY_DIR/client/cli/build.sh $TARGET_BIN_DIR/serverjockey_cmd.pyz || exit 1
