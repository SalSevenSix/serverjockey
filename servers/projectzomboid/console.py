...
def resources(mailer: msgabc.MulticastMailer, resource: httprsc.WebResource):
    ...
    builder.psh('world')
    builder.put('{command}', prcext.ConsoleCommandHandler(mailer, _WORLD), 's')
    builder.pop()
    builder.psh('config')
    builder.psh('options', _OptionsHandler(mailer), 's')
    builder.put('reload', prcext.ConsoleCommandHandler(mailer, _OPTIONS_RELOAD), 's')
    builder.psh('x{option}')
    builder.put('{command}', _OptionCommandHandler(mailer), 's')
    builder.pop().pop().pop()
    builder.psh('players', _PlayersHandler(mailer))
    builder.psh('x{player}')
    builder.put('{command}', prcext.ConsoleCommandHandler(mailer, _PLAYER), 's')
    builder.pop().pop()
    builder.psh('whitelist')
    builder.put('{command}', prcext.ConsoleCommandHandler(mailer, _WHITELIST), 's')
    builder.pop()
    builder.psh('banlist')
    builder.put('{command}', prcext.ConsoleCommandHandler(mailer, _BANLIST), 's')
