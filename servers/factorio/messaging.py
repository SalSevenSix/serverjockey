...
class _PlayerEventSubscriber(msgabc.AbcSubscriber):
    ...
    def handle(self, message):
        if _PlayerEventSubscriber.JOIN_FILTER.accepts(message):
            ...
            self._mailer.post(self, _PlayerEventSubscriber.PLAYER_EVENT, {'event': 'join', 'name': value})
            return None
        if _PlayerEventSubscriber.LEAVE_FILTER.accepts(message):
            ...
            self._mailer.post(self, _PlayerEventSubscriber.PLAYER_EVENT, {'event': 'leave', 'name': value})
            return None
        return None
