...
class Deployment:
    ...
    def resources(self, resource: httprsc.WebResource):
        ...
        builder.put_wipes(self._runtime_dir, dict(
            save=self._save_dir, config=self._config_dir, logs=self._logs_dir, all=self._world_dir))
    ...
    async def _sync_mods(self):
        ...
        for name, entry in data.copy().items():
            ...
            if not synced:
                livedir = await self._sync_mod(logger, name, livedir)
                if livedir:
    ...
    async def _sync_mod(self, logger: msglog.LogPublisher, name: str, livedir: str) -> str | None:
